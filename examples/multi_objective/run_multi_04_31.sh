iteration=300
scenario='multi_04'
log_file="result_${scenario}_dce1.log"
result_file="result_${scenario}_dce1.txt"
csv_file="result_${scenario}_dce1"
sampler_idx=1 # 0 / 1 / 2 / -1 (-1 is for alternate)
sampler_type=dce # demab / dmab / random / dce / halton / udemab
...
do
    if [[ $to_plot == 'True' ]]; then
        python $scenario/$scenario.py -n $iteration --headless -e $csv_file.$seed -sp $scenario/$scenario.scenic -gp $scenario/ -rp $scenario/$scenario\_spec.py -s $sampler_type --seed $seed --using-sampler $sampler_idx -m $simulator --max-simulation-steps $simulation_steps -co $scenario/outputs -o $scenario/outputs >> $scenario/outputs/$log_file
