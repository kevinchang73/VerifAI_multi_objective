...
sampler_idx=2 # 0 / 1 / -1 (-1 is for alternate)
...
do
    if [ "$to_plot" = "True" ]; then
        python $scenario/$scenario.py -n $iteration --headless -e $csv_file.$seed -sp $scenario/$scenario.scenic -gp $scenario/ -rp $scenario/$scenario\_spec.py -s $sampler_type --seed $seed --using-sampler $sampler_idx -m $simulator -co $scenario/outputs --exploration-ratio $exploration_ratio -o $scenario/outputs >> $scenario/outputs/$log_file
