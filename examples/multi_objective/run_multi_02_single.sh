...
for seed in $(seq 0 2);
do
    python $scenario/$scenario.py -n $iteration --headless -e $scenario -sp $scenario/$scenario.scenic -gp $scenario/ -rp $scenario/$scenario\_spec.py -s $sampler_type -o $scenario/outputs --seed $seed --using-sampler $sampler_idx --single-graph > $log_file
...
#-m scenic.simulators.carla.model --max-simulation-steps 600