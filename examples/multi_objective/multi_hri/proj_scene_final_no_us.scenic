...
behavior GrabAndNav():
    ...
    try:
        while True:
            wait
    interrupt when ego.in_position:
        do NavToHuman()
        do SpotHandOver()
        terminate

behavior MoveToJointAngles(joint_angles, steps=50):
    start_pos = np.array(self.articulated_agent.arm_joint_pos)
...
behavior ReachHandAndWalk(walk_position, reach_position):
    ...
    interrupt when spot.holding_object:
...
record spot.holding_object as spot_hold
record ego.in_position as human_in_pos
